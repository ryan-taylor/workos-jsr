import { assertContains, assertEquals, type assertExists, mockFetchError, type mockFetchJson, mockFetchRedirect, restoreFetch } from './test_config.ts';
...
Deno.test('LoginForm - handles error response', async () => {
  ...
  assertEquals(serverError, 'Authentication failed. Please check your credentials.');
  assertEquals(validationError, null);
