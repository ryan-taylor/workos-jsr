...
function readMarkdownSync(path: string): string {
  try {
    ...
    console.warn(
      `Warning: Could not read file ${path}. Creating new file.`,
      error,
    );
    return "";
  }
 ...
 */
function generateGitHubPermalink(
  filePath: string,
  lineNumber?: number,
): string {
  // Remove leading "./" if present
 ...
 */
function generateRegressionGuard(
  testName: string,
  failureCause: string,
): string {
...
function parseExistingBurndown(content: string): {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      inFixHistory && line.startsWith("| ") && !line.includes("Date | Issue")
    ) {
...
function generateMiniTrendChart(data: number[], label: string): string {
  ...
  const chart = data.map((value) => {
    ...
    const index = Math.min(
      Math.floor(normalized * chartChars.length),
      chartChars.length - 1,
    );
...
function generateBurndownDocument(
  ...
  const document = [
    ...
    ...(trends
      ? [
        ...
        "",
        `- **Pass Rate Trend**: ${
          (trends.trends.passRate.change * 100).toFixed(1)
        }% ${trends.trends.passRate.change >= 0 ? "improvement" : "decline"}`,
        `- **Failed Tests Trend**: ${
          Math.abs(trends.trends.failedTests.change)
        } tests ${trends.trends.failedTests.change <= 0 ? "fixed" : "added"}`,
        ...
        generateMiniTrendChart(
          trends.trends.passRate.values.map((v) => v * 100),
          "Pass Rate %",
        ),
        "```",
        "",
  ...
  const failuresByCategory: Record<
    string,
    Array<{ name: string; file: string; errorMessage: string }>
  > = {};
  ...
  for (const [category, failures] of Object.entries(failuresByCategory)) {
    ...
    const displayCategory = category === "Script Error"
      ? "Module Path Issues"
    ...
    failures.forEach((failure) => {
      ...
      if (failure.errorMessage.includes("Module not found")) {
        ...
        suggestedFix =
          "Investigate test setup and async handling. Check for timeouts or race conditions.";
        priority = "High";
      } else {
        suggestedFix =
          "Investigate underlying issue in implementation or test.";
      }

      document.push(
        `| ${displayCategory} | ${linkedFile} | ${
          failure.errorMessage.split("\n")[0]
        } | ${suggestedFix} | ${priority} | ⏳ In Progress |`,
      );
    });
  ...
  burndownData.results
    ...
    .forEach((failure, index) => {
      ...
      if (failure.error?.message?.includes("Module not found")) {
        analysis =
          "The error indicates there's a double file extension (.ts.ts) in the import path. This is likely a typo in the import statement.";
      } else if (failure.error?.message?.includes("cancelled")) {
        analysis =
          "Multiple tests are being cancelled, which could indicate issues with the test setup, timeouts, or underlying implementation problems.";
      } else {
        analysis =
          "This failure requires further investigation to determine the root cause.";
      }
      ...
      document.push(
        `- [GitHub Link](${generateGitHubPermalink(failure.file)})`,
      );
      ...
      if (failure.error?.message?.includes("Module not found")) {
        ...
        suggestedFix =
          "Investigate the underlying issue in the implementation or test.";
      }
      ...
      document.push(
        `**Priority**: High - ${
          failure.error?.category.includes("Security")
            ? "Security-related functionality is critical"
            : "This issue is blocking test execution"
        }.`,
      );

      // Add regression guard section
      document.push(
        generateRegressionGuard(
          failure.name,
          failure.error?.category || "Unknown failure",
        ),
      );
      document.push("");
    });
  ...
  document.push(
    `| ${today} | Generated burndown document | Created automated document generation tool | Improved documentation and tracking |`,
  );
...
function main() {
  try {
    ...
    try {
      ...
      if (trends) {
        console.log(
          `Found ${trends.totalRuns} historical runs for trend analysis`,
        );
      }
    ...
    try {
      ...
      if (existingContent) {
        console.log(
          "Parsing existing burndown document to preserve fix history...",
        );
    ...
    if (existingFixHistory.length > 0) {
      ...
      const fixHistoryIndex = documentLines.findIndex((line) =>
        line === "## Fix History"
      );
        ...
        if (line.startsWith("| ") && line.includes(" | ")) {
          ...
          if (match && match[1]) {
            ...
            if (fixedIssues.has(filePath)) {
              ...
              documentLines[i] = line.replace(
                /\| ⏳ In Progress \|$/,
                `| ${fixedIssues.get(filePath)} |`,
              );
            }
          }
        }
