...
class MockWorkOS {
  ...
  async post<T>(path: string, entity?: unknown, options = {}): Promise<{ data: T }> {
    this.lastPath = path;
  ...
  async put<T>(path: string, entity?: unknown, options = {}): Promise<{ data: T }> {
    this.lastPath = path;
    this.lastMethod = 'put';
    this.lastData = entity;
...
class MockSSO {
  getAuthorizationUrl(options: { connection?: string; organization?: string; domain?: string }) {
...
class MockTelemetryManager {
  ...
  endSpan(spanId: string, status = SpanStatus.OK, message?: string, attributes = {}): void {
  ...
  recordMetric(
    name: string,
    value: number | { count: number; sum: number; buckets: { count: number; upperBound: number }[] },
    type = 'counter',
    attributes = {},
  ): void {
  ...
  recordLog(body: string, severity = 9, attributes = {}, spanId?: string): void {
...
import { instrumentWorkOSCore, instrumentSSO, instrumentDirectorySync, instrumentUserManagement } from "../src/telemetry/instrumentation.ts";
...
Deno.test('Telemetry - instrumentSSO wraps methods correctly', async () => {
  ...
  const trackedManager = {
    ...
    recordMetric: (name: string, value: any, type = 'counter', attributes = {}) => {
      recordMetricCalls++;
      lastMetricName = name;
...
Deno.test('Telemetry - TelemetryManager span lifecycle', () => {
  ...
  // Start a span
  const spanId = manager.startSpan('test.operation', { 'custom.attribute': 'value' });
...
Deno.test('Telemetry - TelemetryManager recordMetric', () => {
  ...
  manager.recordMetric('test.counter', 1, 'counter', { 'custom.attribute': 'value' });
  
  // Verify metric was recorded
  assertEquals(manager.getMetricsCount(), 1);
  
  // Record a gauge metric
  manager.recordMetric('test.gauge', 42.5, 'gauge', { 'custom.attribute': 'value' });
