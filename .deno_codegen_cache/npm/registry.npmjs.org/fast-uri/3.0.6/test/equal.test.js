...
const runTest = (t, suite) => {
  suite.forEach((s) => {
    ...
    t.equal(
      fn(s.pair[0], s.pair[1]),
      s.result,
      `${s.pair[0]} ${operator} ${s.pair[1]}`,
    );
    t.equal(
      fn(s.pair[1], s.pair[0]),
      s.result,
      `${s.pair[1]} ${operator} ${s.pair[0]}`,
    );
  });
};
...
test("URI Equals", (t) => {
  const suite = [
    {
      pair: ["example://a/b/c/%7Bfoo%7D", "eXAMPLE://a/./b/../b/%63/%7bfoo%7d"],
      result: true,
    }, // test from RFC 3986
    {
      pair: ["http://example.org/~user", "http://example.org/%7euser"],
      result: true,
    }, // test from RFC 3987
  ];
  runTest(t, suite);
...
test("HTTPS Equals", (t) => {
  const suite = [
    ...
    {
      pair: ["https://example.com:/", "https://example.com:443/"],
      result: true,
    },
  ];
  runTest(t, suite);
...
test("WSS Equal", (t) => {
  const suite = [
    {
      pair: ["WSS://ABC.COM:443/chat#one", "wss://abc.com/chat"],
      result: true,
    },
  ];
