...
async function readTextDiff(filePath: string): Promise<string> {
  try {
    ...
    throw new Error(`Failed to read text diff from ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function readJsonDiff(filePath: string): Promise<PRCommentOutput> {
  try {
    ...
    throw new Error(`Failed to read or parse JSON diff from ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
function formatAsMarkdown(textDiff: string, jsonDiff: PRCommentOutput): string {
  ...
  if (jsonDiff.breakingChanges.length > 0) {
    ...
    for (const pathChange of jsonDiff.breakingChanges) {
      ...
      for (const op of pathChange.operations) {
        ...
        markdown += `- ${methodIcon} **${op.method.toUpperCase()}** - ${capitalize(op.type)}\n`;
        ...
        if (op.details) {
          ...
          if (op.details.parameters && op.details.parameters.length > 0) {
            ...
            for (const param of op.details.parameters) {
              ...
              markdown += `    - ${changeIcon} \`${param.name}\` (in: ${param.in})${param.isBreaking ? '⚠️' : ''}: ${getChangeDescription(param)}\n`;
            }
          }
          ...
          if (op.details.responses && op.details.responses.length > 0) {
            ...
            for (const resp of op.details.responses) {
              ...
              markdown += `    - ${changeIcon} \`${resp.status}\`${resp.isBreaking ? '⚠️' : ''}: ${getChangeDescription(resp)}\n`;
            }
          }
          ...
          if (op.details.requestBody && op.details.requestBody.length > 0) {
            ...
            for (const req of op.details.requestBody) {
              ...
              markdown += `    - ${changeIcon} \`${req.contentType}\`${req.isBreaking ? '⚠️' : ''}: ${getChangeDescription(req)}\n`;
            }
          }
        }
      }
      ...
      for (const op of pathChange.operations) {
        ...
        if (op.details) {
          ...
          if (op.details.parameters && op.details.parameters.length > 0) {
            ...
            for (const param of op.details.parameters) {
              const changeIcon = getChangeIcon(param.type, param.isBreaking);
              markdown += `    - ${changeIcon} \`${param.name}\` (in: ${param.in}): ${getChangeDescription(param)}\n`;
            }
          }
          ...
          if (op.details.responses && op.details.responses.length > 0) {
            ...
            for (const resp of op.details.responses) {
              const changeIcon = getChangeIcon(resp.type, resp.isBreaking);
              markdown += `    - ${changeIcon} \`${resp.status}\`: ${getChangeDescription(resp)}\n`;
            }
          }
          ...
          if (op.details.requestBody && op.details.requestBody.length > 0) {
            ...
            for (const req of op.details.requestBody) {
              const changeIcon = getChangeIcon(req.type, req.isBreaking);
              markdown += `    - ${changeIcon} \`${req.contentType}\`: ${getChangeDescription(req)}\n`;
            }
          }
        }
      }
  ...
  // Add footer
  markdown += `*Generated at ${new Date().toISOString()} by OpenAPI diff tooling*`;
...
function getChangeDescription(change: any): string {
  ...
  if (type === "added") {
    return change.isBreaking ? "required item added (breaking)" : "optional item added";
  } else if (type === "deleted") {
    return change.isBreaking ? "required item removed (breaking)" : "optional item removed";
  } else {
...
function getPRInfo(): GitHubPRInfo {
  ...
  if (!prNumber && Deno.env.get("GITHUB_EVENT_PATH")) {
    try {
      ...
      console.error(`Failed to extract PR number from event: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  
  if (!prNumber) {
    throw new Error("Could not determine PR number. Set the PR_NUMBER environment variable.");
  }
 ...
 */
async function postCommentUsingGitHubCLI(prInfo: GitHubPRInfo, comment: string): Promise<boolean> {
  try {
    ...
    console.error(`Failed to post comment using GitHub CLI: ${error instanceof Error ? error.message : String(error)}`);
    return false;
  }
}
 ...
 */
async function postCommentUsingCurl(prInfo: GitHubPRInfo, comment: string): Promise<boolean> {
  try {
    ...
    console.error(`Failed to post comment using curl: ${error instanceof Error ? error.message : String(error)}`);
    return false;
  }
}
...
async function postCommentToGitHubPR(comment: string): Promise<boolean> {
  try {
    ...
    console.log(`Attempting to post comment to PR #${prInfo.prNumber} using GitHub CLI...`);
