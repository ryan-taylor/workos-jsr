import { assertEquals, assertContains, assertExists, createMockUser, mockFetch, mockFetchError, mockFetchJson, mockFormSubmitEvent, mockInputEvent, render, restoreFetch } from "./test_config.ts";
...
Deno.test("ProfileForm - successful form submission", async () => {
  ...
  // Mock state setters
  const setSuccessMessage = (msg: string | null) => { successMessage = msg; };
  const setErrorMessage = (msg: string | null) => { errorMessage = msg; };
