  ...
  return {
    ...
    render: (template: string, view: unknown): string => {
      ...
      return template.replace(/\{\{([^}]+)\}\}/g, (_match, key) => 
        (view as Record<string, string>)[key] || '');
    },
...
Deno.test("Mustache Cache - Basic caching functionality", () => {
  ...
  assertEquals(mockMustache.parseCallCount, 1, "Parse should only be called once");
  ...
  assertEquals(mockMustache.parseCallCount, 2, "New template should trigger parse");
});
...
Deno.test("Mustache Cache - Cache size tracking", () => {
  ...
  assertEquals(getTemplateCacheSize(), 2, "Cache size should not change for repeated templates");
  
  // Clear cache
  clearTemplateCache();
  assertEquals(getTemplateCacheSize(), 0, "Cache should be empty after clearing");
});
...
Deno.test("Mustache Cache - Performance tracking", async () => {
  ...
  const result = mockMustache.render(template, { greeting: "Hello", name: "World" });
  ...
  assertEquals(getTemplateCacheSize(), 1, "Template should be cached even with perf tracking disabled");
});