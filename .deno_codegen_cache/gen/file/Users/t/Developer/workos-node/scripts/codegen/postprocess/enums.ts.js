 ...
 */ export const largeBrandedEnumTransform = {
  async process (project, filePath) {
    ...
    const options = {
      ...
      unionMode: ENUM_UNIONS === "branded" || ENUM_UNIONS === "union" ? ENUM_UNIONS : "auto"
    };
    // Get the source file
 ...
 */ function prepareEnumTransformation(enumDecl, options) {
  ...
  const baseName = enumName.endsWith("Enum") ? enumName.replace(/Enum$/, "") : enumName;
  ...
  const useBrandedType = options.unionMode === "branded" || options.unionMode === "auto" && values.length > (options.literalLimit || 45);
  let typeDeclaration;
 ...
 */ function addBrandedImport(sourceFile) {
  ...
  const hasImport = imports.some((imp)=>{
    ...
    return moduleSpecifier?.includes("branded") && imp.getNamedImports().some((named)=>named.getName() === "Branded");
  });
  if (!hasImport) {
