...
async function setupTest() {
  ...
  // These files are necessary for Deno to resolve the imports during type checking
  ...
  await Deno.writeTextFile(
    join(STRICT_OUTPUT, "something.ts"),
    "export interface Something { prop: string; }\n",
  );
  await Deno.writeTextFile(
    join(STRICT_OUTPUT, "otherThing.ts"),
    "export interface OtherThing { value: number; }\n",
  );
  await Deno.writeTextFile(
    join(strictUtilsDir, "things.ts"),
    "export interface YetAnotherThing { items: string[]; }\n",
  );
...
async function runPatchScript(args: string[] = []): Promise<boolean> {
  ...
  try {
    ...
    console.error(`Error importing patch script: ${error}`);
    return false;
  }
}
...
async function verifyFileContent(
  ...
  try {
    ...
    // This covers standard imports, type imports, and dynamic imports
    const importRegex =
    ...
    if (!correctTypes) {
      console.error(
        `❌ ${
          isStrict
            ? "Types not converted to unknown"
            : "Types incorrectly converted"
        }`,
      );
      return false;
    }
...
async function verifyCacheCreated(): Promise<boolean> {
  ...
  for await (const entry of Deno.readDir(CACHE_DIR)) {
    if (entry.isDirectory) {
      for await (const subEntry of Deno.readDir(join(CACHE_DIR, entry.name))) {
        if (subEntry.name.endsWith(".cache")) {
          foundCacheFiles = true;
          break;
        }
      }
    }
    if (foundCacheFiles) break;
  }
...
async function testRaceConditionPrevention(): Promise<boolean> {
  ...
  const [normalSuccess, strictSuccess] = await Promise.all([
    normalPromise,
    strictPromise,
  ]);
  ...
  if (normalVerified && strictVerified) {
    console.log(
      "✅ Race condition test passed! Both normal and strict files correctly processed in parallel",
    );
    return true;
  }
...
async function testCaching(): Promise<boolean> {
  ...
  // Run the patching script first time
  ...
  const cacheVerified = await verifyCacheCreated();
  if (!cacheVerified) {
    console.error("❌ Cache not created correctly");
    return false;
  }
...
import { YetAnotherThing } from "../utils/things";`;
  ...
  try {
    ...
    const { addTsExtensionsToImports } = await import(
      "./patch-openapi-runtime-cached.ts"
    );
    ...
    console.log(
      `Direct function test: ${testPassed ? "✅ PASSED" : "❌ FAILED"}`,
    );
    ...
    console.log(
      `Race Condition Prevention: ${
        raceConditionPassed ? "✅ PASSED" : "❌ FAILED"
      }`,
    );
    console.log(
      `Caching Mechanism: ${cachingPassed ? "✅ PASSED" : "❌ FAILED"}`,
    );
