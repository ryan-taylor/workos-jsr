  ...
  async function handleDeleteDomain(domainId: string) {
    try {
      const response = await fetch(
        `/api/organization-domains/delete?id=${domainId}`,
        {
          method: "DELETE",
        },
      );
                ...
                <form onSubmit={handleUpdateOrganization}>
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      onChange={(e: JSX.TargetedEvent<HTMLInputElement>) =>
                        setEditForm({
                          ...editForm,
                          externalId: (e.target as HTMLInputElement).value,
                        })}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                    />
                  </div>
                  ...
                  <div className="mb-4">
                    <label className="flex items-center">
                      <input
                        type="checkbox"
                        checked={editForm.allowProfilesOutsideOrganization ||
                          false}
                        onChange={(e: JSX.TargetedEvent<HTMLInputElement>) =>
                          setEditForm({
                            ...editForm,
                        ...
                        <div className="mb-4 overflow-hidden border border-gray-200 rounded-md">
                          <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                              {Object.entries(editForm.metadata || {}).map((
                                [key, value],
                              ) => (
                                <tr key={key}>
                    ...
                    {Object.keys(organization.metadata || {}).length > 0
                      ? (
                        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-4">
                          {Object.entries(organization.metadata || {}).map((
                            [key, value],
                          ) => (
                            <div key={key}>
                              <dt className="text-sm font-medium text-gray-500">
