...
class Help {
  ...
  visibleOptions(cmd) {
    ...
    // Implicit help
    const showShortHelpFlag = cmd._hasHelpOption && cmd._helpShortFlag &&
      !cmd._findOption(cmd._helpShortFlag);
    const showLongHelpFlag = cmd._hasHelpOption &&
      !cmd._findOption(cmd._helpLongFlag);
...
class Command extends EventEmitter {
  ...
  addHelpCommand(enableOrNameAndArgs, description) {
    if (enableOrNameAndArgs === false) {
      ...
      this._helpCommandDescription = description ||
  ...
  _hasImplicitHelpCommand() {
    if (this._addImplicitHelpCommand === undefined) {
      return this.commands.length && !this._actionHandler &&
  ...
  addOption(option) {
    ...
    if (
      option.negate || option.optional || option.required ||
      ...
      if (val !== null && option.parseArg) {
        try {
          ...
          if (err.code === "commander.invalidOptionArgument") {
            const message =
              `error: option '${option.flags}' argument '${val}' is invalid. ${err.message}`;
  ...
  _executeSubCommand(subcommand, args) {
    ...
    let bin = path.basename(scriptPath, path.extname(scriptPath)) + "-" +
      subcommand._name;
 ...
 - if the default executable name is not suitable, use the executableFile option to supply a custom name`;
  ...
  _parseCommand(operands, unknown) {
    ...
    if (operands && this._findCommand(operands[0])) {
      ...
      this._hasImplicitHelpCommand() && operands[0] === this._helpCommandName
    ) {
      ...
      if (
        this.commands.length && this.args.length === 0 &&
        !this._actionHandler && !this._defaultCommandName
      ) {
  ...
  _checkForMissingMandatoryOptions() {
    ...
    for (let cmd = this; cmd; cmd = cmd.parent) {
      cmd.options.forEach((anOption) => {
        if (
          anOption.mandatory &&
          (cmd._getOptionValue(anOption.attributeName()) === undefined)
        ) {
    ...
    while (args.length) {
      ...
      if (
        ...
        if (this._findCommand(arg)) {
          ...
          arg === this._helpCommandName && this._hasImplicitHelpCommand()
        ) {
          operands.push(arg);
  ...
  helpInformation(contextOptions) {
    ...
    if (helper.helpWidth === undefined) {
      helper.helpWidth = (contextOptions && contextOptions.error)
  ...
  outputHelp(contextOptions) {
    ...
    groupListeners.slice().reverse().forEach((command) =>
      command.emit("beforeAllHelp", context)
    );
    ...
    if (deprecatedCallback) {
      ...
      if (
        typeof helpInformation !== "string" && !Buffer.isBuffer(helpInformation)
      ) {
  ...
  help(contextOptions) {
    ...
    if (
      exitCode === 0 && contextOptions &&
      typeof contextOptions !== "function" && contextOptions.error
    ) {
      exitCode = 1;
    }
...
function outputHelpIfRequested(cmd, args) {
  const helpOption = cmd._hasHelpOption &&
