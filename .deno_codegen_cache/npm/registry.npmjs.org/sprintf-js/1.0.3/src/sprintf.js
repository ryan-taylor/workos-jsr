  ...
  sprintf.format = function (parse_tree, argv) {
    ...
    for (i = 0; i < tree_length; i++) {
      ...
      if (node_type === "string") {
        ...
        if (
          re.not_string.test(match[8]) && re.not_json.test(match[8]) &&
        ...
        if (re.json.test(match[8])) {
          ...
          pad_character = match[4]
            ? match[4] === "0" ? "0" : match[4].charAt(1)
            : " ";
          pad_length = match[6] - (sign + arg).length;
          pad = match[6]
            ? (pad_length > 0 ? str_repeat(pad_character, pad_length) : "")
            : "";
