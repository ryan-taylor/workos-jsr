...
export default function WidgetsShowcase({
  ...
  const [activeTab, setActiveTab] = useState<WidgetType>(
    WidgetType.USERS_TABLE,
  );
  ...
  const handleGenerateWidget = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to generate widget configuration: ${response.status}`,
        );
      }
      ...
      <div class="bg-white shadow rounded-lg p-6">
        ...
        <div class="mb-4">
          ...
          <select
            ...
            onChange={(e) =>
              setWidgetTheme(
                (e.target as HTMLSelectElement).value as WidgetTheme,
              )}
            class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
          >
          ...
          {showCustomCss && (
            <div class="mt-2">
              <textarea
                ...
                value={customCss}
                onInput={(e) =>
                  setCustomCss((e.target as HTMLTextAreaElement).value)}
      ...
      {widgetToken && (
        <div class="bg-white shadow rounded-lg p-6">
          ...
          <div class="space-y-4">
            ...
            <h3 class="text-md font-medium text-gray-800 mt-4">
              Server-side Token Code
            </h3>
            <div class="relative">
              <pre class="bg-gray-800 text-gray-100 p-3 rounded-md text-sm overflow-x-auto">
                {getServerCode()}
