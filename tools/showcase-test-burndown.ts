...
function main() {
  try {
    console.log(
      "Test Burndown System - Historical Tracking & Trend Analysis Showcase",
    );
    console.log(
      "--------------------------------------------------------------------",
    );
    ...
    if (historyFiles.length === 0) {
      console.log(
        "No historical data found. Please run analyze-test-burndown.ts first.",
      );
      console.log(
        "This will generate and store test burndown data in the history directory.",
      );
      return;
    }
    ...
    if (recentRun) {
      ...
      console.log(
        `Pass Rate: ${(recentRun.analysis.summary.passRate * 100).toFixed(1)}%`,
      );
    ...
    if (trends) {
      ...
      console.log(
        `Date range: ${trends.datePeriod.start} to ${trends.datePeriod.end}`,
      );
      console.log(
        `Pass rate trend: ${
    ...
    if (consistencyAnalysis) {
      console.log(
        `Analyzing consistency across ${consistencyAnalysis.totalRuns} test runs`,
      );
      console.log(
    ...
    if (velocityAnalysis) {
      console.log(
        `Analyzing velocity across ${velocityAnalysis.dataPoints.length} test runs`,
      );

      if (velocityAnalysis.recent.direction === "improving") {
        console.log(
          `Current fix rate: ${
            velocityAnalysis.recent.netFixRate.toFixed(2)
          } tests per day`,
        );

        if (velocityAnalysis.projections.daysToZeroFailures !== "never") {
          console.log(
            `Estimated completion in ${velocityAnalysis.projections.daysToZeroFailures} days (${velocityAnalysis.projections.estimatedCompletionDate})`,
          );
          console.log(
            `Confidence level: ${velocityAnalysis.projections.confidenceLevel}`,
          );
        } else {
        ...
        console.log(
          "Warning: Test failures are increasing rather than decreasing",
        );
        console.log(
          `Regression rate: ${
            Math.abs(velocityAnalysis.recent.netFixRate).toFixed(2)
          } new failures per day`,
        );
      } else {
        console.log(
          "Test failure count is stable (not improving or regressing)",
        );
      }
