...
export const handler: Handlers = {
  async GET(req, ctx) {
    ...
    let authFactors = [];
    try {
      const factorsResult = await userManagement.listAuthFactors({
        userId: user.id,
      });
      ...
      if (action === "update_profile") {
        ...
        if (!currentPassword || !newPassword || !confirmPassword) {
          return new Response(null, {
            status: 302,
            headers: {
              Location: "/account?changePassword=true&error=missing_fields",
            },
          });
        }
        ...
        if (newPassword !== confirmPassword) {
          return new Response(null, {
            ...
            headers: {
              Location:
                "/account?changePassword=true&error=passwords_dont_match",
            },
          });
        }
        ...
        try {
          ...
          return new Response(null, {
            status: 302,
            headers: {
              Location: "/account?changePassword=true&error=invalid_password",
            },
          });
        }
      }
