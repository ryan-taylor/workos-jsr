...
export default function EventsStream(
  { initialEvents, eventTypes }: EventsStreamProps,
) {
    ...
    try {
      ...
      if (data.status === "success") {
        ...
        if (isPolling && events.length > 0) {
          ...
          const newEvents = data.data.filter((e: Event) =>
  ...
  const getEventColor = (eventType: string): string => {
    ...
    if (eventType.includes("authenticated")) {
                ...
                <div class="space-y-6">
                  {events.map((event) => (
                    <div key={event.id} class="relative pl-10">
                      {/* Timeline dot */}
                      <div class="absolute left-3 top-5 w-3 h-3 rounded-full bg-blue-500 transform -translate-x-1.5">
                      </div>
