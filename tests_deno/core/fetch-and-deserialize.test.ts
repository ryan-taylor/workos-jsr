...
class MockWorkOS {
  ...
  async get<T>(path: string, options?: Record<string, unknown>): Promise<{ data: T }> {
    this.lastPath = path;
    this.lastMethod = 'get';
...
Deno.test('fetchAndDeserialize - handles single item response with positional parameters', async () => {
  ...
  assertEquals((result as { data: Record<string, unknown>[] }).data[0].id, '123');
});
...
Deno.test('fetchAndDeserialize - applies pagination options with positional parameters', async () => {
  ...
  const lastRequest = mockWorkos.getLastRequest();
  assertEquals((lastRequest.options?.query as Record<string, unknown>)?.limit, 10);
  assertEquals((lastRequest.options?.query as Record<string, unknown>)?.order, 'asc');
});
...
Deno.test('fetchAndDeserialize - uses "desc" as default order when not specified', async () => {
  ...
  const lastRequest = mockWorkos.getLastRequest();
  assertEquals((lastRequest.options?.query as Record<string, unknown>)?.order, 'desc');
});
...
Deno.test('fetchAndDeserialize - validates required parameters', async () => {
  ...
  await assertRejects(
    async () => {
      await fetchAndDeserialize(mockWorkos as unknown as WorkOS, undefined, mockDeserializer);
    },
    Error,
  ...
  await assertRejects(
    async () => {
      await fetchAndDeserialize(mockWorkos as unknown as WorkOS, '/items', undefined);
    },
    Error,
    'Endpoint and deserializer function must be provided'
  );
