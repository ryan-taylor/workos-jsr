  ...
  const createRole = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || `Failed to create role: ${response.status}`,
        );
      }
  ...
  const updateRole = async () => {
    ...
    try {
      const response = await fetch(
        `/api/roles/update?id=${uiState.editingRole.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const deleteRole = async (roleId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || `Failed to delete role: ${response.status}`,
        );
      }
  ...
  const assignRole = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || `Failed to assign role: ${response.status}`,
        );
      }
  ...
  const unassignRole = async (userId: string, roleId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || `Failed to unassign role: ${response.status}`,
        );
      }
    ...
    <div class="p-4">
      ...
      <div class="border-b border-gray-200 mb-4">
        <nav class="-mb-px flex space-x-8">
          <button
            onClick={() =>
              setUIState((prev) => ({ ...prev, activeTab: "roles" }))}
          ...
          <button
            onClick={() =>
              setUIState((prev) => ({ ...prev, activeTab: "assignment" }))}
            class={`py-2 px-1 border-b-2 font-medium text-sm ${
          ...
          <button
            onClick={() =>
              setUIState((prev) => ({ ...prev, activeTab: "templates" }))}
            class={`py-2 px-1 border-b-2 font-medium text-sm ${
        ...
        <div>
          ...
          <div class="mb-6 bg-white p-4 rounded shadow">
            ...
            <p class="text-sm text-gray-600 mb-4">
              This table shows a simulated view of user role assignments. In a
              real application, this would display actual role assignments from
              the WorkOS API.
            </p>
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                  ...
                  <tr>
                    ...
                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                      <button
                        onClick={() =>
                          unassignRole("user_01", "role_01HXYZ123456789")}
                        class="text-red-600 hover:text-red-900"
                      >
                        Remove
                      </button>
                  ...
                  <tr>
                    ...
                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                      <button
                        onClick={() =>
                          unassignRole("user_02", "role_02HXYZ123456789")}
                        class="text-red-600 hover:text-red-900"
                      >
                        Remove
                      </button>
                  ...
                  <tr>
                    ...
                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                      <button
                        onClick={() =>
                          unassignRole("user_03", "role_03HXYZ123456789")}
                        class="text-red-600 hover:text-red-900"
                      >
                        Remove
                      </button>
        ...
        <div>
          <div class="mb-4">
            ...
            <p class="text-sm text-gray-600">
              Use these pre-defined role templates as a starting point for
              creating new roles in your application.
            </p>
          </div>
                ...
                <div class="mt-2 space-y-2 max-h-48 overflow-y-auto border rounded p-2">
                  {permissions.map((permission) => (
                    <div key={permission.key} class="flex items-start">
                      ...
                      <div class="ml-3 text-sm">
                        <label
                          for={`permission-edit-${permission.key}`}
                          class="font-medium text-gray-700"
                        >
                          {permission.key}
                        </label>
          ...
          <div class="bg-white rounded-lg max-w-md w-full overflow-hidden shadow-xl">
            ...
            <div class="p-6">
              <p class="mb-4 text-gray-700">
                Are you sure you want to delete the role "{uiState.editingRole
