...
export default function FGAPolicyEditor() {
  ...
  const newRelationship = useSignal<
    { source: string; relation: string; target: string; description: string }
  >({
    source: "",
    relation: "",
    target: "",
  ...
  const handleAddObjectType = () => {
    ...
    if (
      modelData.value.objectTypes.some((ot: ObjectType) =>
        ot.name === newObjectType.value.name
      )
    ) {
      validationError.value =
        `Object type '${newObjectType.value.name}' already exists`;
      return;
    }
  ...
  const handleAddRelationship = () => {
    if (
      !newRelationship.value.source || !newRelationship.value.relation ||
    ...
    const sourceExists = modelData.value.objectTypes.some((ot: ObjectType) =>
      ot.name === newRelationship.value.source
    );
    const targetExists = modelData.value.objectTypes.some((ot: ObjectType) =>
      ot.name === newRelationship.value.target
    );

    if (!sourceExists) {
      validationError.value =
        `Source type '${newRelationship.value.source}' doesn't exist`;
      return;
    }

    if (!targetExists) {
      validationError.value =
        `Target type '${newRelationship.value.target}' doesn't exist`;
      return;
    }
  ...
  const handleDeleteObjectType = (index: number) => {
    ...
    const isUsed = modelData.value.relationships.some(
      (rel: Relationship) =>
        rel.source === objectType.name || rel.target === objectType.name,
    );

    if (isUsed) {
      validationError.value =
        `Cannot delete object type '${objectType.name}' as it's used in one or more relationships`;
      return;
    }
  ...
  const handleSavePolicy = async () => {
    try {
      ...
      alert(
        "In a real implementation, this would save the policy to WorkOS FGA",
      );
  ...
  return (
    <div class="fga-policy-editor">
      <h2 class="text-2xl font-bold mb-4">
        Fine-Grained Authorization Policy Editor
      </h2>
      <p class="text-gray-600 mb-6">
        Create and edit authorization models with objects and their
        relationships.
      </p>
      ...
      <div class="example-selector mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            ...
            <p class="text-sm text-gray-600 mt-1">
              Documents, folders, and access levels
            </p>
          </button>
        ...
        <div class="policy-editor-panel">
          ...
          <div class="object-types-section mb-8">
            ...
            <p class="text-sm text-gray-600 mb-4">
              Define the types of objects in your authorization model
            </p>

            <div class="object-types-list mb-4">
              {modelData.value.objectTypes.map((
                objectType: ObjectType,
                index: number,
              ) => (
                <ObjectTypeItem
          ...
          <div class="relationships-section">
            ...
            <p class="text-sm text-gray-600 mb-4">
              Define the relationships between object types
            </p>

            <div class="relationships-list mb-4">
              {modelData.value.relationships.map((
                relationship: Relationship,
                index: number,
              ) => (
                <RelationshipItem
              ...
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-2">
                <div>
                  ...
                  >
                    ...
                    {modelData.value.objectTypes.map((ot: ObjectType) => (
                ...
                <div>
                  ...
                  <input
                    ...
                    onInput={(e) =>
                      newRelationship.value = {
                        ...newRelationship.value,
                        relation: (e.target as HTMLInputElement).value,
                      }}
              ...
              {modelData.value.objectTypes.length > 0
                ? (
                  <div class="text-center text-gray-500">
                    <p>
                      This would render an interactive graph visualization of
                ...
                : (
                  <p class="text-gray-500">
                    Add object types and relationships to visualize your
                    authorization model
                  </p>
                )}
            </div>
