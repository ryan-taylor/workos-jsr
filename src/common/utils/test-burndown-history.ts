...
import { TestBurndownData, TestBurndownAnalysis } from "./test-burndown-analyzer.ts";
...
export function getHistoricalEntries(maxEntries?: number): string[] {
  try {
    ...
    console.warn(`Warning: Could not read history directory ${HISTORY_DIR}:`, error);
    return [];
  }
}
...
export function readHistoricalEntry(filepath: string): HistoricalEntry | null {
  try {
    ...
    console.warn(`Warning: Could not read historical entry ${filepath}:`, error);
    return null;
  }
}
...
export function analyzeTrends(maxRuns = 10): TrendAnalysisResult | null {
  ...
  const failedTests = historicalData.map(entry => 
    entry.burndownData.failed
  );
  ...
  const failedTestsChange = failedTests[failedTests.length - 1] - failedTests[0];
...
export function formatTrendReport(analysis: TrendAnalysisResult): string {
  ...
  report.push(`### Failed Tests: ${analysis.trends.failedTests.first} → ${analysis.trends.failedTests.last}`);
  report.push("");
  if (analysis.trends.failedTests.change < 0) {
    report.push(`🟢 **Improving** (${Math.abs(analysis.trends.failedTests.change)} tests fixed)`);
  } else if (analysis.trends.failedTests.change > 0) {
    report.push(`🔴 **Regression** (${analysis.trends.failedTests.change} new failures)`);
  } else {
    report.push(`🟠 **Stable** at ${analysis.trends.failedTests.last} failures`);
  }
  report.push("");
  
  // Add total tests trend
  report.push(`### Total Tests: ${analysis.trends.totalTests.first} → ${analysis.trends.totalTests.last}`);
  report.push("");
  if (analysis.trends.totalTests.change > 0) {
    ...
    report.push(`ℹ️ **Decreasing** by ${Math.abs(analysis.trends.totalTests.change)} tests`);
  } else {
    report.push(`ℹ️ **Stable** at ${analysis.trends.totalTests.last} total tests`);
  }
  report.push("");
  ...
  for (let i = 0; i < analysis.trends.passRate.values.length; i++) {
    ...
    report.push(`| ${dateStr} | ${failedTests} | ${passRate}% | ${totalTests} |`);
  }
