...
async function simulateNavigation(
  ...
  for (const route of routes) {
    ...
    const request = sessionId
      ? sessionProvider.createRequestWithSession(route, sessionId)
      : new Request(route);
...
function createProtectedRouteContext(
  url: string,
  sessionId?: string,
...
async function simulateSessionExpiry(
  ...
  const match = new RegExp(`${SESSION_OPTIONS.cookieName}=([^;]+)`).exec(
    cookie || "",
  );
...
Deno.test("Session Management - Auth state persists across route navigation", async () => {
  ...
  const responses = await simulateNavigation(
    routes,
    sessionProvider,
    sessionId,
  );
  ...
  for (const response of responses) {
    assertEquals(
      response.status,
      200,
      "User should remain authenticated across routes",
    );
  }
  ...
  if (sessionId) {
    for (const route of routes) {
      const request = sessionProvider.createRequestWithSession(
        route,
        sessionId,
      );
      ...
      assertEquals(
        currentUser?.id,
        user.id,
        "User ID should persist across routes",
      );
      assertEquals(
        currentUser?.email,
        user.email,
        "User email should persist across routes",
      );
    }
  }
});
...
Deno.test("Session Management - Session expiry handling works correctly", async () => {
  ...
  if (sessionId) {
    ...
    assertExists(
      authResponse,
      "Should get a redirect response when session expired",
    );
    if (authResponse) {
      assertEquals(
        authResponse.status,
...
Deno.test("Session Management - Authentication token refresh works", async () => {
  ...
  if (sessionId) {
    const refreshedSession = await simulateSessionExpiry(
      sessionProvider,
      sessionId,
      user,
    );
