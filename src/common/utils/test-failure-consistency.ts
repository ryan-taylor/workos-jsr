 ...
 */
export function analyzeFailureConsistency(
  maxRuns = 10,
): ConsistencyAnalysisResult | null {
  ...
  historicalData.forEach((entry) => {
    ...
    results
      ...
      .forEach((result) => {
        ...
        if (
          !testFailures[testId].errorMessages.some((msg) =>
            msg.includes(shortenedError)
          )
        ) {
...
function identifyRecentlyChangedTests(
  historicalData:
    (import("./test-burndown-history.ts").HistoricalEntry | null)[],
  findFixed: boolean,
): TestFailureData[] {
  ...
  if (!findFixed) {
    for (const testId in failuresInRecent) {
      if (
        !failuresInEarlier[testId] && failuresInRecent[testId].failureRate > 0.5
      ) {
...
export function formatConsistencyReport(
  ...
  if (analysis.consistentlyFailing.length === 0) {
    ...
    report.push(
      "These tests fail in most or all runs and likely represent systemic issues that should be prioritized:",
    );
    report.push("");
    ...
    analysis.consistentlyFailing.forEach((test) => {
      ...
      report.push(
        `| ${test.name} | ${test.file} | ${failureRate} | ${categories} |`,
      );
    });
  ...
  if (analysis.intermittentlyFailing.length === 0) {
    ...
    report.push(
      "These tests fail inconsistently and may indicate race conditions, timing issues, or environmental dependencies:",
    );
    report.push("");
    report.push("| Test | File | Failure Rate | Categories |");
    report.push("|------|------|-------------|------------|");
  ...
  if (analysis.recentlyFixed.length > 0) {
    ...
    report.push("");
    report.push(
      "These tests were failing in earlier runs but have been fixed in recent runs:",
    );
    report.push("");
  ...
  if (analysis.recentlyBroken.length > 0) {
    ...
    report.push("");
    report.push(
      "These tests were previously passing but have started failing in recent runs:",
    );
    report.push("");
    report.push("| Test | File |");
    report.push("|------|------|");
  ...
  if (analysis.consistentlyFailing.length > 0) {
    ...
    report.push(
      "2. Look for patterns in failure categories to identify systemic issues",
    );
    report.push(
      "3. Consider temporarily disabling tests that cannot be fixed immediately",
    );
    report.push("");
  }
  ...
  if (analysis.intermittentlyFailing.length > 0) {
    ...
    report.push("");
    report.push(
      "1. Add additional logging to capture state at time of failure",
    );
    report.push(
      "2. Look for race conditions, timing issues, or external dependencies",
    );
    report.push(
      "3. Consider increasing timeouts or adding retry mechanisms for tests with timing issues",
    );
    report.push(
      "4. Isolate tests that may be affecting each other when run in sequence",
    );
    report.push("");
  }
