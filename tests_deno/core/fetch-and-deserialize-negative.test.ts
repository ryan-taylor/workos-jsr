...
class MockWorkOS {
  ...
  constructor(mockResponse: unknown, errorConfig: Record<string, unknown> = {}) {
...
Deno.test('fetchAndDeserialize - handles 404 Not Found errors with positional parameters', async () => {
  ...
  try {
    ...
    assertEquals((error as any).message, 'The requested resource was not found');
  }
});
...
Deno.test('fetchAndDeserialize - handles malformed response data with legacy options object', async () => {
  const mockWorkos = new MockWorkOS({ data: [null, undefined, 123, 'not-an-object'] });
...
Deno.test('fetchAndDeserialize - handles nested error structure', async () => {
  ...
  try {
    ...
    assertEquals(nestedApiError.error.message, 'An internal server error occurred');
