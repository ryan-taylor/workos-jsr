...
function hasSignificantChanges(diffResult: OasDiffResult): boolean {
  ...
  if (diffResult.paths) {
    ...
    if (
      diffResult.paths.modified?.some((path) =>
  ...
  if (diffResult.endpoints) {
    if (
      diffResult.endpoints.added?.length ||
 ...
 */
function formatDiffForComment(
  diffResult: OasDiffResult,
  baseFile: string,
  revisionFile: string,
): string {
  ...
  if (diffResult.paths) {
    ...
    if (diffResult.paths.added?.length) {
      ...
      for (const path of diffResult.paths.added) {
        ...
        if (path.operations?.added?.length) {
          comment += `  - Methods: ${
            path.operations.added.map((op) => `\`${op.toUpperCase()}\``).join(
              ", ",
            )
          }\n`;
        }
    ...
    if (diffResult.paths.deleted?.length) {
      comment +=
        `### 🔴 Deleted Paths (${diffResult.paths.deleted.length})\n\n`;
      for (const path of diffResult.paths.deleted) {
        ...
        if (path.operations?.deleted?.length) {
          comment += `  - Methods: ${
            path.operations.deleted.map((op) => `\`${op.toUpperCase()}\``).join(
              ", ",
            )
          }\n`;
        }
    ...
    if (diffResult.paths.modified?.length) {
      const modifiedWithOperationChanges = diffResult.paths.modified.filter(
        (path) =>
      ...
      if (modifiedWithOperationChanges.length > 0) {
        comment +=
          `### 🟠 Modified Paths with Operation Changes (${modifiedWithOperationChanges.length})\n\n`;
        ...
        for (const path of modifiedWithOperationChanges) {
          ...
          // Deleted operations
          if (path.operations?.deleted?.length) {
            comment += `  - 🔴 Deleted methods: ${
              path.operations.deleted.map((op) => `\`${op.toUpperCase()}\``)
                .join(", ")
            }\n`;
          }
          ...
          if (modifiedOps.length > 0) {
            comment += `  - 🟠 Modified methods: ${
              modifiedOps.map((op) => `\`${op.toUpperCase()}\``).join(", ")
            }\n`;
          }
        }
        comment += `\n`;
...
async function main() {
  try {
    ...
    const commentContent = formatDiffForComment(
      diffResult,
      baseSpec,
      revisionSpec,
    );
    ...
    console.log(
      `\n${
        hasChanges
          ? "🟠 API path/verb changes detected"
          : "✅ No significant API path/verb changes"
      }`,
    );
