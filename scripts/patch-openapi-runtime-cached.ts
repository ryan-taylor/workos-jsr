...
async function getGeneratorVersion(): Promise<string> {
  try {
    ...
    const packageJson = JSON.parse(
      await Deno.readTextFile(join(".", "package.json")),
    );
...
async function processFile(filePath: string): Promise<boolean> {
  try {
    ...
    if (cachedContent !== null) {
      ...
      if (!hasAllExtensions) {
        console.warn(
          `Warning: Cache retrieved but not all imports have .ts extensions in: ${filePath}`,
        );
    ...
    if (content !== newContent) {
      ...
      if (!hasAllExtensions) {
        console.warn(
          `Warning: Extensions were lost after writing to: ${filePath}`,
        );
