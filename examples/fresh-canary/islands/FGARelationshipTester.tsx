              ...
              <div class={`mt-6 p-4 rounded-lg ${
                ...
                <div class="flex items-center">
                  ...
                  <div class="ml-4 text-sm text-gray-600">
                    Can <span class="font-medium">{subjectType.value}:{subjectId.value}</span> {" "}
                    have relation <span class="font-medium">{relation.value}</span> {" "}
                    with <span class="font-medium">{resourceType.value}:{resourceId.value}</span>?{" "}
            ...
            <div class="mt-6 bg-white border border-gray-200 rounded-lg shadow-sm p-6">
              ...
              <div class="overflow-auto max-h-60">
                {checkHistory.map((historyItem, index) => (
                  <div key={index} class={`p-3 mb-2 rounded ${
                    historyItem.result 
                    ...
                    <p class="text-sm text-gray-700 mt-1">
                      <span class="font-medium">{historyItem.check.subjectType}:{historyItem.check.subjectId}</span> {" "}
                      to <span class="font-medium">{historyItem.check.relation}</span> {" "}
                      with <span class="font-medium">{historyItem.check.resourceType}:{historyItem.check.resourceId}</span>
                    </p>
                  </div>
                ))}
              </div>
        ...
        <div>
          <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
            ...
            <div class="space-y-4">
              {exampleScenarios.map((example, index) => (
                <div key={index} class="border border-gray-200 rounded p-4 hover:bg-gray-50 transition cursor-pointer"
                     onClick={() => loadExample(example)}>
                  <h4 class="font-medium text-blue-700">{example.name}</h4>
                  <p class="text-sm text-gray-600 mt-1">{example.description}</p>
                  <div class="mt-2 text-xs text-gray-500">
                    <p><span class="font-medium">Resource:</span> {example.check.resourceType}:{example.check.resourceId}</p>
                    <p><span class="font-medium">Relation:</span> {example.check.relation}</p>
                    <p><span class="font-medium">Subject:</span> {example.check.subjectType}:{example.check.subjectId}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div class="mt-6 bg-white border border-gray-200 rounded-lg shadow-sm p-6">
            ...
            <div class="text-sm text-gray-600 space-y-3">
              <p>
                Fine-grained authorization (FGA) allows you to define complex permissions models
                based on relationships between objects.
              </p>
              <p>
                A check verifies if a subject (like a user) has a specific relation (like "viewer")
                with a resource (like a document).
              </p>
              <p>
                The result tells you whether the relationship exists according to your defined
                authorization model.
              </p>
            </div>
          </div>
