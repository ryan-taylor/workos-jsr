...
import { type DirectoryUserWithGroups, getDirectory, initDirectorySync, listDirectoryUsers } from '../../utils/directory-sync.ts';
...
export const handler: Handlers<DirectoryUsersPageData> = {
  async GET(req, ctx) {
    ...
    try {
      ...
      const result = await listDirectoryUsers(workos, { directory: directoryId });
      ...
      return ctx.render({
        ...
        users: [],
        error: error instanceof Error ? error.message : 'Failed to load directory users',
      });
    }
  },
};

const DirectoryUsersPage: FunctionComponent<PageProps<DirectoryUsersPageData>> = ({ data }) => {
  ...
  return (
    <div class='p-4 mx-auto max-w-screen-lg'>
                    ...
                    <td class='whitespace-nowrap px-3 py-4 text-sm text-gray-500'>
                      <span
                        class={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          user.state === 'active' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                        }`}
                      >
                        {user.state}
                      </span>
      ...
      <div class='mt-8'>
        <h2 class='text-xl font-semibold mb-2'>Using Directory Users in Your Application</h2>
        <div class='bg-gray-50 p-4 rounded-lg'>
          <p class='mb-2'>Directory Sync provides user data from your identity provider that you can use to:</p>
          <ul class='list-disc pl-5 space-y-1'>
            <li>Automatically provision user accounts</li>
            <li>Map directory groups to application roles or permissions</li>
            <li>Keep user attributes and access in sync with your identity provider</li>
            <li>Receive real-time updates via webhooks when users or group memberships change</li>
          </ul>
        </div>
      </div>
    </div>
