 ...
 */
export function analyzeBurndownVelocity(maxRuns = 10): VelocityAnalysisResult | null {
 ...
 */
function calculateVelocityMetrics(start: VelocityDataPoint, end: VelocityDataPoint): VelocityMetrics {
...
function calculateConsistencyScore(dataPoints: VelocityDataPoint[]): number {
  ...
  for (let i = 1; i < dataPoints.length; i++) {
    ...
    const daysBetween = (current.date.getTime() - previous.date.getTime()) / (1000 * 60 * 60 * 24);
  ...
  const mean = dailyVelocities.reduce((sum, v) => sum + v, 0) / dailyVelocities.length;
...
export function formatVelocityReport(analysis: VelocityAnalysisResult): string {
  ...
  report.push("");
  report.push(`- **Days Analyzed**: ${analysis.recent.daysBetween.toFixed(1)} days`);
  report.push(`- **Net Change**: ${analysis.recent.netChange} tests`);
  
  if (analysis.recent.direction === "improving") {
    report.push(`- **Current Velocity**: ${analysis.recent.velocity.toFixed(2)} tests fixed per day`);
    report.push(`- **Status**: 🟢 Improving`);
  } else if (analysis.recent.direction === "regressing") {
    report.push(`- **Current Velocity**: ${Math.abs(analysis.recent.velocity).toFixed(2)} new failures per day`);
  ...
  if (analysis.recent.direction === "improving" && analysis.projections.daysToZeroFailures !== "never") {
    report.push(`- **Estimated Days to Completion**: ${analysis.projections.daysToZeroFailures}`);
    report.push(`- **Target Completion Date**: ${analysis.projections.estimatedCompletionDate}`);
    report.push(`- **Confidence Level**: ${analysis.projections.confidenceLevel}`);
  } else {
    report.push("*Unable to project completion date with current velocity.*");
    
    if (analysis.recent.direction === "regressing") {
      report.push("");
      report.push("⚠️ **Warning**: Test failures are increasing rather than decreasing.");
    } else if (analysis.recent.direction === "stable") {
      report.push("");
      report.push("ℹ️ **Note**: Test failure count is not improving. Need to increase velocity to make progress.");
    }
  }
  ...
  report.push("");
  report.push(`- **Total Duration**: ${analysis.overall.daysBetween.toFixed(1)} days`);
  report.push(`- **Starting Failed Tests**: ${analysis.dataPoints[0].failed}`);
  report.push(`- **Current Failed Tests**: ${analysis.dataPoints[analysis.dataPoints.length - 1].failed}`);
  report.push(`- **Net Change**: ${analysis.overall.netChange} tests`);
  report.push(`- **Average Velocity**: ${analysis.overall.velocity.toFixed(2)} tests per day`);
  report.push(`- **Consistency Score**: ${(analysis.consistency * 100).toFixed(0)}%`);
  report.push("");
  ...
  for (let i = 0; i < analysis.dataPoints.length; i++) {
    ...
    report.push(`| ${dateStr} | ${point.failed} | ${passRate} | ${netChange} |`);
  }
  ...
  if (analysis.recent.direction === "improving") {
    report.push("- Maintain current fix rate to meet projected completion date");
    report.push("- Focus on consistently failing tests first for maximum impact");
    report.push("- Continue tracking velocity to ensure progress stays on track");
  } else if (analysis.recent.direction === "regressing") {
    ...
    report.push("- Increase focus on fixing failing tests to achieve positive velocity");
