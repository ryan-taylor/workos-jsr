...
function main() {
  try {
    ...
    if (analysis.summary.passRate < (1 - config.maxFailureRate)) {
      ...
      violations.push(
        `Failure rate of ${failureRate} exceeds threshold of ${
          config.maxFailureRate.toFixed(2)
        }`,
      );
    }
    ...
    if (analysis.summary.failed > config.maxFailureCount) {
      violations.push(
        `Failure count of ${analysis.summary.failed} exceeds threshold of ${config.maxFailureCount}`,
      );
    }
    ...
    if (trends && trends.trends.passRate.change < -config.maxRegressionRate) {
      ...
      violations.push(
        `Regression of ${regression} exceeds threshold of ${
          config.maxRegressionRate.toFixed(2)
        }`,
      );
    }
    ...
    if (config.strictMode && trends && trends.trends.passRate.change < 0) {
      violations.push(
        `Any regression violates strict mode: ${
          Math.abs(trends.trends.passRate.change).toFixed(2)
        }`,
      );
    }
    ...
    if (violations.length > 0) {
      ...
      console.log(
        `- Current pass rate: ${(analysis.summary.passRate * 100).toFixed(1)}%`,
      );
      console.log(`- Failed tests: ${analysis.summary.failed}`);

      if (trends) {
        const changeDirection = trends.trends.passRate.change >= 0
          ? "improvement"
          : "regression";
        console.log(
          `- Trend: ${
            Math.abs(trends.trends.passRate.change * 100).toFixed(1)
          }% ${changeDirection} since last run`,
        );
      }
