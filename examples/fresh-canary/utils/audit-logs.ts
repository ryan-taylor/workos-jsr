...
import { AuditLogExportOptions } from "../../../src/audit-logs/interfaces/audit-log-export-options.interface.ts";
import { AuditLogExport } from "../../../src/audit-logs/interfaces/audit-log-export.interface.ts";
...
export async function getMockAuditLogs(options: {
  ...
  if (options.rangeStart) {
    filteredEvents = filteredEvents.filter(event =>
      event.occurredAt >= options.rangeStart!
    );
  }
  
  if (options.rangeEnd) {
    filteredEvents = filteredEvents.filter(event =>
      event.occurredAt <= options.rangeEnd!
    );
  }
