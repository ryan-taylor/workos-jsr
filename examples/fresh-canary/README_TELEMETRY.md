...
This guide explains how to use and configure the built-in OpenTelemetry integration in the WorkOS SDK.
...
Telemetry data is exported using the OpenTelemetry Protocol (OTLP) over HTTP, which can be collected by any OTLP-compatible backend.
...
- `authenticateWithPassword` - Tracking authentication attempts and success rates
...
The WorkOS SDK includes a built-in telemetry dashboard for Fresh applications. To use it:

1. Add the dashboard to your application by copying `TelemetryDashboard.tsx` to your islands directory
...
For production use, you'll want to set up an OpenTelemetry Collector to receive and process telemetry data.

### Docker Compose Setup

Here's a simple docker-compose configuration for an OpenTelemetry Collector with Prometheus and Jaeger:
...
Once you have data flowing into your telemetry backend, you can create custom dashboards in Grafana.
...
1. **No data in collector**: Ensure telemetry is enabled and the endpoint is correct
...
Each platform has specific exporters that can be configured in the OpenTelemetry Collector.
