...
Deno.test('Portal - generateLink with required parameters', async () => {
  ...
  assertEquals((lastRequest.data as Record<string, unknown>).intent, GeneratePortalLinkIntent.SSO);
  assertEquals((lastRequest.data as Record<string, unknown>).organization, 'org_123');
  assertEquals((lastRequest.data as Record<string, unknown>).return_url, undefined);
  assertEquals((lastRequest.data as Record<string, unknown>).success_url, undefined);
});
...
Deno.test('Portal - generateLink with all parameters', async () => {
  ...
  assertEquals(lastRequest.path, '/portal/generate_link');
  assertEquals(lastRequest.method, 'post');
  assertEquals((lastRequest.data as Record<string, unknown>).intent, GeneratePortalLinkIntent.DSync);
  assertEquals((lastRequest.data as Record<string, unknown>).organization, 'org_456');
  assertEquals((lastRequest.data as Record<string, unknown>).return_url, 'https://example.com/return');
  assertEquals((lastRequest.data as Record<string, unknown>).success_url, 'https://example.com/success');
});
