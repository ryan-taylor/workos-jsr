...
const DirectorySyncList: FunctionComponent<DirectorySyncListProps> = (
  { initialDirectories },
) => {
  // State using signals
  ...
  const sortField = useSignal<
    "name" | "domain" | "state" | "type" | "createdAt"
  >("name");
  ...
  const toggleSort = (
    field: "name" | "domain" | "state" | "type" | "createdAt",
  ) => {
  ...
  const SortIndicator = (
    { field }: { field: "name" | "domain" | "state" | "type" | "createdAt" },
  ) => {
                  ...
                  >
                    ...
                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                      <span
                        class={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          getStatusBadgeClass(directory.state)
                        }`}
                      >
                        {directory.state}
                      </span>
                    ...
                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                      {directory.formattedDate ||
                        new Date(directory.createdAt).toLocaleString()}
                    </td>
