...
async function main(): Promise<void> {
  try {
    ...
    try {
      ...
      if (!stat.isDirectory) {
        console.error(
          `Error: Spec directory '${config.specDir}' is not a directory`,
        );
        Deno.exit(1);
      }
    } catch (error) {
    ...
    if (specFiles.length === 1) {
      console.log(
        "Warning: Only one spec file found, nothing to compare against",
      );
      Deno.exit(0);
    }
    ...
    if (dialectChanged) {
      ...
      if (config.postComment) {
        ...
        commentContent +=
          `The OpenAPI specification dialect has changed from \`${previousDialect.version}\` to \`${latestDialect.version}\`.\n\n`;

        // Add advice based on version change
        if (
          latestDialect.version !== "unknown" &&
          previousDialect.version !== "unknown"
        ) {
          if (upgradeType === "Upgrade") {
            commentContent +=
              `This is a **version upgrade**. You may need to:\n`;
            commentContent +=
              `- Run \`deno task codegen:upgrade\` to handle the upgrade process\n`;
            commentContent +=
              `- Update any custom templates for compatibility\n`;
            commentContent += `- Review generated code for breaking changes\n`;
          } else if (upgradeType === "Downgrade") {
            commentContent +=
              `This is a **version downgrade**. This is unusual and may indicate an issue with:\n`;
