...
export async function verifySpec(
  ...
  try {
    ...
    const { checksum: newProcessedChecksum } =
    ...
    if (opts.verifyRawChecksum && result.storedRawChecksum) {
      result.rawChecksumMatches =
        result.storedRawChecksum === result.currentRawChecksum;
    ...
    if (opts.verifyProcessedChecksum && result.storedProcessedChecksum) {
      result.processedChecksumMatches =
        result.storedProcessedChecksum === result.currentProcessedChecksum;
    ...
    if ((hasRawMismatch || hasProcessedMismatch) && opts.updateOnMismatch) {
      ...
      updatedSpec["x-spec-processed-checksum"] =
        result.currentProcessedChecksum;
    ...
    if (
      ...
      error.message.includes("API specification drift detected")
    ) {
      throw error; // Re-throw our specific error
    }

    // Handle other errors
    const errorMessage = `Error verifying OpenAPI spec at ${specPath}: ${
...
if (import.meta.main) {
  try {
    ...
    if (Deno.args.length < 1) {
      console.error(
        "Usage: deno run -A verify-spec.ts <path-to-spec-file> [--no-fail] [--update]",
      );
      Deno.exit(1);
    }
