...
class Widgets {
  ...
  async getToken({
    ...
    const { data } = await this.workos.post<{ token: string }>(
      "/widgets/token",
      {
...
Deno.test("Widgets - getToken with required parameters", async () => {
  ...
  assertEquals(
    (lastRequest.data as Record<string, unknown>).organization_id,
    "org_123",
  );
  assertEquals(
    (lastRequest.data as Record<string, unknown>).user_id,
    "user_123",
  );
...
Deno.test("Widgets - getToken with all parameters", async () => {
  ...
  assertEquals((lastRequest.data as Record<string, unknown>).scopes, [
