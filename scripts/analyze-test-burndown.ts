...
import { analyzeBurndownData, formatAnalysisReport, TestBurndownData, TestBurndownAnalysis } from "../src/common/utils/test-burndown-analyzer.ts";
import { storeHistoricalData, createTimestampedFilename, HISTORY_DIR, analyzeTrends } from "../src/common/utils/test-burndown-history.ts";
...
function main() {
  try {
    ...
    if (trendAnalysis) {
      console.log(`Trend analysis completed with data from ${trendAnalysis.totalRuns} runs`);
    } else {
    ...
    if (velocityAnalysis && velocityAnalysis.overall.netFixRate > 0) {
      console.log(`\nBurndown Velocity: ${velocityAnalysis.recent.velocity.toFixed(2)} tests fixed per day`);
      if (velocityAnalysis.projections.daysToZeroFailures !== "never") {
        console.log(`Estimated completion: ${velocityAnalysis.projections.daysToZeroFailures} days (${velocityAnalysis.projections.estimatedCompletionDate})`);
      }
    }
    ...
    console.log(`- Performance Outliers: ${analysis.performanceOutliers.length}`);
    console.log(`- Potentially Flaky Tests: ${analysis.potentiallyFlakyTests.length}`);
