...
import { FreshSessionProvider, SessionOptions } from "./fresh-session-provider.ts";
...
Deno.test("FreshSessionProvider", async (t) => {
  ...
  await t.step("should seal and unseal data correctly", async () => {
    // Seal the data
    const sealed = await provider.sealData(testData, { password: testPassword });
    ...
    const unsealed = await provider.unsealData(sealed, { password: testPassword });
  ...
  await t.step("should support password rotation", async () => {
    ...
    const sealed = await provider.sealData(testData, { password: rotatedPasswords });
    
    // Unseal with rotated passwords
    const unsealed = await provider.unsealData(sealed, { password: rotatedPasswords });
    assertEquals(unsealed, testData);
  });
  ...
  await t.step("should create session response with cookie", async () => {
    ...
    const response = await provider.createSessionResponse(testData, baseOptions);
  ...
  await t.step("should create middleware that adds session to state", async () => {
