...
import type { WebhookEvent, VerifyOptions } from './interfaces/index.ts';
import { SignatureVerificationException } from '../common/exceptions/signature-verification.exception.ts';

export class Webhooks {
  static constructEvent(payload: string, signature: string, secret: string): WebhookEvent {
    const data = JSON.parse(payload);
    return deserializeWebhookEvent(data);
  }

  static async verifySignature({ payload, signature, secret, tolerance = 180 }: VerifyOptions): Promise<void> {
    const timestamp = Date.now() / 1000;
    const providedSignature = signature.split(',')[1];

    if (!providedSignature) {
      throw new SignatureVerificationException('No signature found with expected format');
    }
    ...
    if (Math.abs(timestampAge) > tolerance) {
      throw new SignatureVerificationException('Timestamp outside tolerance zone');
    }
  }
}