...
Deno.test("Passwordless - createSession with required parameters", async () => {
  ...
  assertEquals(
    (lastRequest.data as Record<string, unknown>).email,
    "user@example.com",
  );
  assertEquals(
    (lastRequest.data as Record<string, unknown>).redirect_uri,
    undefined,
  );
  assertEquals(
    (lastRequest.data as Record<string, unknown>).expires_in,
    undefined,
  );
});
...
Deno.test("Passwordless - createSession with all parameters", async () => {
  ...
  assertEquals(
    (lastRequest.data as Record<string, unknown>).state,
    "custom-state",
  );
  assertEquals(
    (lastRequest.data as Record<string, unknown>).connection,
    "connection_123",
  );
