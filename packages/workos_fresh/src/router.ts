 ...
 */
export async function makeRouter(routes: Route[]): Promise<(req: Request) => Promise<Response>> {
  ...
  if (version === 1) {
    ...
    try {
      const freshModule = await import('$fresh/server.ts') as Fresh1.ServerModule;
      ...
      return (_req: Request) => Promise.resolve(new Response('Router initialization failed', { status: 500 }));
    }
  } else {
    ...
    try {
      ...
      let appHandler: (req: Request) => Promise<Response> = (_req: Request) => {
        return Promise.resolve(new Response('App not initialized', { status: 500 }));
      };
