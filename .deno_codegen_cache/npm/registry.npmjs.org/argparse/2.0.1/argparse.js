...
function _string_split(string, sep, maxsplit) {
  ...
  if (result.length > maxsplit) {
    result = result.slice(0, maxsplit).concat([
...
function _choices_to_array(choices) {
  if (choices === undefined) {
    ...
    choices !== null && typeof choices[Symbol.iterator] === "function"
  ) {
...
function _parse_opts(args, descriptor) {
  ...
  if (
    typeof last_opt === "object" && last_opt !== null &&
  ...
  if (args.length) {
    let from = Object.entries(descriptor).filter(([k, v]) =>
      k[0] !== "*" && v !== no_default
    ).length;
    let to = Object.entries(descriptor).filter(([k]) =>
      k[0] !== "*"
    ).length;
    throw new TypeError(
      sub(
...
// Formatting Help
// ===============
const HelpFormatter = _camelcase_alias(_callable(
  class HelpFormatter {
    /*
  ...
  class FileType extends Function {
    ...
    [util.inspect.custom]() {
      ...
      let args_str = []
        ...
        .concat(
          kwargs.filter(([/*kw*/, arg]) => arg !== undefined)
...
const _ActionsContainer = _camelcase_alias(_callable(
    ...
    add_argument() {
      ...
      if ("_get_formatter" in this) {
        try {
          ...
          if (
            err instanceof TypeError && err.message !== "invalid nargs value"
          ) {
    ...
    class ArgumentParser extends _AttributeHolder(_ActionsContainer) {
      ...
      _parse_known_args(arg_strings, namespace) {
        ...
        for (let mutex_group of this._mutually_exclusive_groups) {
          ...
          for (
            let [i, mutex_action] of Object.entries(mutex_group._group_actions)
          ) {
        ...
        while (start_index <= max_option_string_index) {
          ...
          let next_option_string_index = Math.min(
            ...
            ...Object.keys(option_string_indices).map(Number).filter((index) =>
              index >= start_index
            ),
          );
      ...
      _read_args_from_files(arg_strings) {
        ...
        for (let arg_string of arg_strings) {
          ...
          if (
            !arg_string || !this.fromfile_prefix_chars.includes(arg_string[0])
          ) {
      ...
      _match_arguments_partial(actions, arg_strings_pattern) {
        ...
        for (let i of range(actions.length, 0, -1)) {
          ...
          let pattern = actions_slice.map((action) =>
            this._get_nargs_pattern(action)
          ).join("");
      ...
      _get_option_tuples(option_string) {
        ...
        let chars = this.prefix_chars;
        if (
          chars.includes(option_string[0]) && chars.includes(option_string[1])
        ) {
          if (this.allow_abbrev) {
            ...
            for (
              let option_string of Object.keys(this._option_string_actions)
            ) {
      ...
      _get_values(action, arg_strings) {
        ...
        if (!arg_strings.length && action.nargs === OPTIONAL) {
          ...
          !arg_strings.length && action.nargs === ZERO_OR_MORE &&
          !action.option_strings.length
        ) {
