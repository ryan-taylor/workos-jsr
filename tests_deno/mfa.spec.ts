...
Deno.test("MFA - enrollFactor SMS success", async () => {
  ...
  assertEquals(
    (lastRequest.data as Record<string, unknown>).phone_number,
    "+15551234567",
  );
});
...
Deno.test("MFA - enrollFactor TOTP success", async () => {
  ...
  const mockFactorResponse = {
    ...
    totp_uri:
      "otpauth://totp/WorkOS:user@example.com?secret=JBSWY3DPEHPK3PXP&issuer=WorkOS",
  };
  ...
  assertEquals(
    (lastRequest.data as Record<string, unknown>).totp_issuer,
    "WorkOS",
  );
  assertEquals(
    (lastRequest.data as Record<string, unknown>).totp_user,
    "user@example.com",
  );
});
...
Deno.test("MFA - challengeFactor with required parameters", async () => {
  ...
  assertEquals(lastRequest.method, "post");
  assertEquals(
    (lastRequest.data as Record<string, unknown>).sms_template,
    undefined,
  );
});
...
Deno.test("MFA - verifyChallenge handles verification failure", async () => {
  ...
  assertEquals(lastRequest.method, "post");
  assertEquals(
    (lastRequest.data as Record<string, unknown>).code,
    "wrong-code",
  );
});
