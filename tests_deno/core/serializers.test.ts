...
Deno.test('Common Serializers - serialize handles nested objects', () => {
  ...
  assertEquals((serialized.nested as Record<string, unknown>).date, '2023-05-12T12:00:00.000Z');
});
...
Deno.test('Common Serializers - deserializeList handles server list format', () => {
  ...
  assertEquals((deserialized.data[0] as Record<string, unknown>).name, 'Item 1');
