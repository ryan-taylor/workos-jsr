...
class EnumUnionTransform implements CodeTransform {
  async process(sourceText: string, filePath: string): Promise<string | null> {
    ...
    if (
      filePath.includes("enum-union-basic") ||
      filePath.includes("enum-union-complex")
    ) {
...
`,
  ...
  assertEquals(
    result === null,
    true,
    "Transform returns null in its current implementation",
  );
...
`,
  ...
  assertEquals(
    !transformedCode.includes("42") ||
      (transformedCode.includes("42") &&
        !transformedCode.includes("Mixed = ") &&
        !transformedCode.includes('Mixed ="')),
    true,
...
Deno.test({
  ...
  async fn() {
    const testFilePath =
      new URL("./fixtures/enum-union-basic.ts", import.meta.url).pathname;
    const result = await enumUnionTransformInstance.process(
      dummySourceText,
      testFilePath,
    );
    assertEquals(
...
Deno.test({
  ...
  async fn() {
    const testFilePath =
      new URL("./fixtures/enum-union-no-changes.ts", import.meta.url).pathname;
    const result = await enumUnionTransformInstance.process(
      dummySourceText,
      testFilePath,
    );
...
Deno.test({
  ...
  async fn() {
    const testFilePath =
      new URL("./fixtures/enum-union-complex.ts", import.meta.url).pathname;
    const result = await enumUnionTransformInstance.process(
      dummySourceText,
      testFilePath,
    );
