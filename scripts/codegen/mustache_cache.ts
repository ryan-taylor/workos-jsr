...
async function reportRenderTime(
  ...
  if (templateKey in baselines.templates) {
    ...
    if (renderTime > baseline * 2) {
      const message = `Template rendering exceeds 2x baseline: ${renderTime.toFixed(2)}ms vs ${baseline.toFixed(2)}ms baseline`;
 ...
 */
function getCachedTemplate(template: string, parseFunction: (template: string) => unknown): unknown {
...
function createCachedRenderFunction(
  originalRender: (template: string, view: unknown, partials?: Record<string, string>) => string,
  parseFunction: (template: string) => unknown
): (template: string, view: unknown, partials?: Record<string, string>) => string {
  return function cachedRender(
    ...
    if (!bypassPerf) {
      ...
      reportRenderTime(template, renderTime).catch((err) => {
        console.error(`Failed to report render time: ${err instanceof Error ? err.message : String(err)}`);
      });
    }
...
export function enhanceMustacheWithCaching(
  ...
  mustacheObj.render = createCachedRenderFunction(originalRender, mustacheObj.parse);
...
export async function setupMustacheCaching(): Promise<void> {
  ...
  try {
    ...
    console.log("Call enhanceMustacheWithCaching() when Mustache instance is available");
  } catch (error) {
    console.error("Failed to setup Mustache caching:", error instanceof Error ? error.message : String(error));
  }
}