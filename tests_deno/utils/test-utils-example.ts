...
testGroup.test("mockResponse returns the configured response", async () => {
  ...
  console.assert(
    fetchUtils.url() === "https://api.example.com/data",
...
testWithContext(
  "mockResponseOnce returns the configured response once",
  async () => {
    ...
    try {
      ...
      console.assert(
        data1.first === true,
        "First response should have first=true",
      );
      ...
      console.assert(
        data2.second === true,
        "Second response should have second=true",
      );
      ...
      console.assert(
        calls[0][0] === "https://api.example.com/first",
        "First call should be to /first",
      );
      console.assert(
        calls[1][0] === "https://api.example.com/second",
...
testWithContext(
  "mockFetch can be used with custom implementation",
  async () => {
    const uninstall = setupFetchMock();
    try {
      ...
      console.assert(
        requestSpy.calls.length === 1,
        "Request spy should be called once",
      );
      console.assert(
        requestSpy.calls[0][0] === "https://api.example.com/custom",
...
Deno.test("spy tracks function calls and returns configured values", () => {
  ...
  console.assert(
    greet.calls[0][0] === "Alice",
    "First call should be with 'Alice'",
  );
  console.assert(
    greet.calls[1][0] === "Bob",
    "Second call should be with 'Bob'",
  );
  ...
  console.assert(
    result4 === "Hey Dave!",
    "Result should use new implementation",
  );
