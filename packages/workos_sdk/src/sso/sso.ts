...
import type { Profile, Connection, GetAuthorizationUrlOptions } from './interfaces/index.ts';
...
export class SSO {
  ...
  getAuthorizationUrl(options: GetAuthorizationUrlOptions): string {
    const params = new URLSearchParams(serializeGetAuthorizationUrlOptions(options) as Record<string, string>);
