 ...
 * It walks through all .ts files in the entire codebase,
 * finds import statements with paths ending in .ts.ts, and fixes them to end with just .ts.
 *
 * Usage: deno run --allow-read --allow-write scripts/fix-all-duplicate-extensions.ts
 */
...
async function main() {
  ...
  console.log(`Walking through all TypeScript files in ${rootDir}...`);

  for await (
    const entry of walk(rootDir, {
      exts: ["ts"],
      skip: [/node_modules/, /\.git/, /dist/, /build/, /cov/],
    })
  ) {
    if (entry.isFile) {
      filesProcessed++;
  ...
  if (filesChanged > 0) {
    console.log(`\nFiles with fixed duplicate extensions:`);
