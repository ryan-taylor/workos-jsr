...
class TRBL {
  ...
  getValueSequence(declaration, count) {
    const values = [];
    let iehack = "";
    const hasBadValues = declaration.value.type !== "Value" ||
    ...
    function attemptToAdd() {
      ...
      if (side) {
        ...
        for (let i = 0; i < 4; i++) {
          for (const key in sides) {
            if (
              sides[key] !== null && sides[key].special !== values[i].special
            ) {
              return false;
            }
          }
...
function restructBlock(ast, indexer) {
  ...
  cssTree.walk(ast, {
    ...
    enter(node) {
      ...
      const ruleId = (node.pseudoSignature || "") + "|" +
