...
Deno.test({
  ...
  async fn() {
    try {
      ...
      assertEquals(typeof finalVerifyResult.processedChecksumMatches, "boolean");
    } finally {
      // Clean up test files
      try {
  ...
  async fn() {
    try {
      ...
      const modifiedSpecWithOriginalChecksums = {
        ...
        "x-spec-processed-checksum": specWithChecksum["x-spec-processed-checksum"],
      };
      ...
      assertExists(verifyResult.messages.find(m => m.includes("Raw file checksum mismatch detected")));
      assertExists(verifyResult.messages.find(m => m.includes("Processed content checksum mismatch detected")));
...
Deno.test({
  ...
  async fn() {
    try {
      ...
      assertExists(verifyResult.messages.find(m => m.includes("Checksums have been updated")));
      ...
      assertEquals(updatedSpec["x-spec-content-sha"], verifyResult.currentRawChecksum);
      assertEquals(updatedSpec["x-spec-processed-checksum"], verifyResult.currentProcessedChecksum);
      
      // Verify that a subsequent verification works without errors
      const secondVerifyResult = await verifySpec(modifiedSpecPath, { failOnMismatch: false });
      ...
      assertEquals(typeof secondVerifyResult.currentProcessedChecksum, "string");
...
Deno.test({
  ...
  async fn() {
    try {
      ...
      assertExists(verifyResult.messages.find(m => m.includes("not found in the spec")));
...
Deno.test({
  ...
  async fn() {
    try {
      ...
      assertExists(rawOnlyResult.messages.find(m => m.includes("Raw file checksum")));
      ...
      const processedOnlyResult = await verifySpec(modifiedSpecPath, processedOnlyOptions);
      
      // Verify that only processed checksum was verified
      assertEquals(processedOnlyResult.rawChecksumMatches, null);
      assertEquals(typeof processedOnlyResult.processedChecksumMatches, 'boolean');
      assertExists(processedOnlyResult.messages.find(m => m.includes("Processed content checksum")));
    } finally {
      // Clean up test files
      try {
        await Deno.remove(specPath);
